...
if ($output==null) {
	$error = new OEReturn("error03",GetErrMessage(null,$WEInfoPage,"FormNotConfigure"));
...
{
	$OutputWECaptcha = json_decode($OEConfWECaptcha);
}
if (!empty($OEConfWEGoogleRecaptcha)) {
	...
	if (!empty($paramsRecaptchas)) {
		foreach ($paramsRecaptchas as $idRecaptcha => $paramsRecaptcha) {
			$recaptchaScriptVersion = $paramsRecaptcha->ScriptVersion;
			require_once __DIR__ . "/WEGoogleRecaptcha-v{$recaptchaScriptVersion}.php";
			break;
		}
	}
	...
	switch($ElemType) {
		...
		case 'WECaptcha';
			if (WECaptchaCheck($val,$FormLinksElement->ID,$OEVersion)==false) {
				...
				$ErrorText=GetErrMessage(null,$WEInfoPage,"CaptchaError");//"Captcha incorrect!";
				//Gestion des erreurs de déserialisation
		...
		case 'WEGoogleRecaptcha':
			$recaptcha = new OERecaptcha(null, $FormLinksElement->ID);
			if (!$recaptcha->isOk()) { // validation failed
				$ErrorText=GetErrMessage(null,$WEInfoPage,"RecaptchaError"); // "Recaptcha error - please reload (refresh) the page and try again";
				$error = new OEReturn("",$ErrorText);
				echo json_encode($error);
				exit(0);
			}
...
if (($LocalizableString->Get($SendMailInfo->AdminEmail,$Culture)=="")  || ($SendMailInfo->ReturnEmail=="") || ($SendMailInfo->Subject=="") ) {
	$error = new OEReturn("error04",GetErrMessage(null,$WEInfoPage,"FormNotConfigure")); //"Attention, vous n'avez pas configuré le formulaire"
...
if (!$mail->CheckEmail($ReceiptEmail)) {
	$error = new OEReturn("success",GetErrMessage(null,$WEInfoPage,"NoAcknowledgment")); //"Impossible d'envoyer l'accusé de réception : e-mail incorrect"
	echo json_encode($error);
	exit(0);
}
